...
type SuperAdministratorInfo struct {
	...
	IdCardNumber uint64 `json:"id_card_number"` //	1、若管理员类型为法人，则该身份证号码需与法人身份证号码一致。若管理员类型为经办人， 则可填写实际经办人的身份证号码。 2、可传身份证、来往内地通行证、来往大陆通行证、护照等证件号码。3、超级管理员签约时，校验微信号绑定的银行卡实名信息，是否与该证件号码一致。
	Phone        uint64 `json:"phone"`          //请填写管理员的手机号，11位数字， 用于接收微信支付的重要管理信息及日常操作验证码
	Mail         string `json:"mail"`           //1、用于接收微信支付的开户邮件及日常业务通知。 2、需要带@，遵循邮箱格式校验 。
}
type SpecialQualification struct {
	PicFileList []string `json:"pic_file_list"` //图片media id数组，每项的格式同pic_file
}
...
type NameInfo struct {
	NickName         string   `json:"nick_name"`                    //1）小程序名称可以由中文、数字、英文、空格及部分特殊符号(“空格”、“-”、“+”、“&”、“.”)组成。长度在4-30个字符之间，一个中文字等于2个字符。 2）公众号、小程序在微信公众平台上的名称是唯一的，且属于同一主体下，可以重名。3）不得与不同主体的公众号名称重名。
